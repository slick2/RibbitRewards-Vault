	...
	$("#walletConfirmSend").click(function(){
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			if(dvalue>=total){
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+data.value+" RBR unable to send "+total+" RBR").fadeOut().fadeIn();
			}
	...
	function walletBalance(){
		...
		coinjs.addressBalance($("#walletAddress").html(),function(data){
			if($(data).find("result").text()==1){
				...
				$("#walletBalance").html(v+" RBR").attr('rel',v).fadeOut().fadeIn();
			} else {
				$("#walletBalance").html("0.00 RBR").attr('rel',v).fadeOut().fadeIn();
			}
