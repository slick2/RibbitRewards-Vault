...
# Blockchain APIs for bitcore (Multicoin fork)
...
```sh
npm install git://github.com/RibbitFROG/bitcore-explorers-multi.git
bower install bitcore-explorers-multi
...
If you want to query a coin besides Bitcoin you can specify the name in your Insight() call like:

```javascript
var explorers = require('bitcore-explorers');
var insight = new explorers.Insight("ribbit");

insight.getUnspentUtxos('RnVFFNL...', function(err, utxos) {
  if (err) {
    // Handle errors...
  } else {
    // Maybe use the UTXOs to create a transaction
